...
const repo: EffectFunctionRepository = {
  ...
  'Target:Bleed': effectFun(
    ...
    (params) => (origin, play, currentState) => [play, (params.target !== 'Player' && currentState.enemies[params.target]!!.hp > 0) && (params.lifespan > 0) ? actions.addEotEffect(currentState, origin, effect({ display: `Bleed ${play.enemies[params.target]!!.lore.name} [${params.target + 1}]`, range: allRanges, priority: 4, effect: 'Target:Bleed', parameters: { ...params, lifespan: params.lifespan - 1 } })) : currentState],
  ),
